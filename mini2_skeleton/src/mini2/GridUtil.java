...
public class GridUtil
{
   ...
   */   
  // in essence it is determining whether or not the points that the aro array encompasses exists in the arr array
  public static int[][] getSubArray(int[][] arr, int centerRow, int centerCol, int radius, boolean wrapped)
  {
	  int[][] aro = new int[2*radius+1][2*radius+1];
	  if(aro.length > arr.length || aro[0].length > arr[0].length)
	  {
	  ...
	  if(wrapped)
	  {
		  for(int i = centerRow-radius; centerRow+radius >= i; i++)
		  {
			  for(int b = centerCol-radius; centerCol+radius >= b; b++)
			  {
				  if(i < 0 && b < 0)
				  {
					  aro[arrRow][arrCol] = arr[arr.length-i][arr[0].length-b];
				  }
				  else if(i > arr.length && b > arr[0].length)
				  {
					  aro[arrRow][arrCol] = arr[0][0];
				  }
				  else if(i < 0)
				  {
					  aro[arrRow][arrCol] = arr[i+arr.length][b];
				  }
				  else if(b < 0)
				  {
					  aro[arrRow][arrCol] = arr[i][b+arr[0].length];
				  }
				  else if(i >= arr.length)
				  {
					  aro[arrRow][arrCol] = arr[0][b];
				  }
				  else if(b >= arr[0].length)
				  {
					  aro[arrRow][arrCol] = arr[i][0];
				  }
				  else if(b < arr[0].length && i < arr.length)
				  {
					  aro[arrRow][arrCol] = arr[i][b];
				  }
				  arrCol++;
			  }
			  arrRow++;
